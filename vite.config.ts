...
import { browserPairingPlugin } from "./scripts/browser-pairing-plugin";
...
export default defineConfig(async ({ mode }) => {
  const relayUrl = loadEnv(mode, process.cwd(), "POSTURE_").POSTURE_CLOUD_RELAY_URL;
  return {
    plugins: [react(), soundOptionsPlugin(), ...(relayUrl ? [] : [browserPairingPlugin()])],
    ...
    server: {
      proxy: relayUrl ? { "/api/pairing": { target: relayUrl, ws: true } } : undefined,
      port: 1420,
      strictPort: true,
      host: host || false,
      hmr: host
